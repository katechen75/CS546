...
router.post("/search", async (req, res) => {
    ...
    try {
        ...
        if (!allPosts){
            res.status(500).render('users/homePage', {error: 'No posts for this search'});
            return;
        }
        res.render('users/homePage', {posts:allPosts});
    } catch (e) {
...
router.post("/comments/:id", async (req, res) => {
    ...
    try {
        ...
        if (!createComments){
            res.status(500).render('users/homePage', {error: 'Unable to create comment, please log in.'});
            return;
        ...
        res.status(400).render('users/homePage', {error: 'unable to create comment, please log in.'});
        return;
    }
});
